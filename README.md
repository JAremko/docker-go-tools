...
*Keep in mind that this image populates the `/goroot` volume if it's empty. To avoid slow start up it's a good idea to run at least one container in background - it prevents docker from flushing the `/goroot` volume.*
